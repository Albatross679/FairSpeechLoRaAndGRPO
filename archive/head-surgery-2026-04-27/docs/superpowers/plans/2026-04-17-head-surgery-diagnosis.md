...
**Goal:** Build the diagnosis-only pipeline from [tasks/prd-head-surgery-diagnosis.md](../../../tasks/prd-head-surgery-diagnosis.md): per-head attention masking on Whisper-large-v3's decoder, a 640-cell sweep over the Indian-accent utterance subset from Common Voice 25 (**N = 484** — see Dataset drift note below; plan originally targeted 511 on CV24), scoring that ranks heads by their hallucination-reduction Δ (with paired-bootstrap significance and a composite non-Indian-WER regression guard over 422 sampled non-Indian utterances), plus a 36-config decoding-ablation grid and an energy-VAD arm over silence-injected audio. Fine-tuning is explicitly out of scope.

**Architecture:** New `scripts/head_surgery/` package. Primitives: per-head forward-pre-hook on Whisper decoder `self_attn.out_proj` (serial + batched-condition variants). Staged rollout: **establish CV25 baseline** (Gate G1 — redefined from midterm reproduction; see Dataset drift note) → empirical batch-size tune (G1.5) → serial pilot sweep (G2) → batched full sweep with a correctness gate against the serial pilot (G3) → score (G5) → adjunct arms → aggregate report. Reuses the existing `scripts/inference/run_inference.py` Whisper loader and `scripts/analysis/whisper_hallucination_analysis.py` insertion classifier.
...
**Dataset drift note:** The midterm reports 511 Indian-accent utterances on Common Voice 24. CV24 is unavailable here; we use CV25 (`datasets/cv-corpus-25.0-2026-03-09/en/test.tsv`). The strict pure-India single-label filter yields 510 rows on CV25 (plus 9 compound-label rows ignored for purity). **The B2 tarball is truncated** (`gzip: unexpected end of file` reproducibly on two independent downloads of the same 81,491,329,024-byte file); 26 late-positioned clips are unreachable. We therefore use the intersection of (a) strict pure-India and (b) on-disk presence, **N = 484**. Task 2's snapshot JSON has a `note` field documenting this intersection; `scripts/head_surgery/repro_config.py:EXPECTED_N_INDIAN_ACCENT_IDS = 484`. **Gate G1 is redefined** from "reproduce 9.62% within ±0.5pp" to "establish the CV25 baseline insertion rate"; the CV24 midterm number remains in the code as a historical reference. The non-Indian regression-guard subset (Task 11) is similarly capped at the 422 utterances the tarball could yield out of a 500-sample target.
